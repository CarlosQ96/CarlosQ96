# Hey, I'm Carlos Quintero

Software engineer with 8 years of experience, strong backend background (Ruby on Rails, Typescript), now building across Web3 and AI. Enthusiast about multiple technologies like Flutter, Godot and Elixir.

## Portfolio of Projects:

- **[Sippy](https://www.sippy.lat/)** — Stablecoin payments through WhatsApp (ETHOnline 2025 Finalist) (Typescript, AI Agents, Web3)
- **[Camello](https://camello.xyz/)** — AI agents that sell, support, and grow your business across WhatsApp (Typescript, AI Agents)
- **[Galeon](https://www.galeon.finance/)** — Compliance-ready privacy for real-world payments (Mantle global hackathon winner) (Adonisjs, Web3)
- **[Giveth](https://giveth.io/)** — Donation platform for public goods (Javascript Backend)
- **[Prisma Dids](https://github.com/prisma-collective/dids)** - Decentralized Identity framework for Cardano, Winner of Cardano Grant. (Javascript, Cardano blockchain web3)
- **[Do it Center](https://www.doitcenter.com.pa/)** — Shopify ecommerce for one of Panama's largest home-improvement retailers, including the backend of the mobile app. (Ruby on Rails + Shopify)
- **[Tuily](https://www.koombea.com/blog/es/tuily-una-innovadora-solucion-fintech-para-pymes/)** — Fintech cards and expense management for small and medium businesses (Ruby on Rails, Fintech)
- **[FlightLogger](https://www.koombea.com/case-studies/flightlogger)** — Mobile app for real-time flight tracking and itinerary management (Ruby on rails)
- **[Astrea](https://github.com/CarlosQ96/Astrea)** - Mobile app made with flutter, with an AI agent to handle reminders for their users with Claude. (Flutter, AI Agents)

## Stack

- **Languages:** Ruby, JavaScript, TypeScript, Solidity
- **Backend:** Ruby on Rails, Node.js, PostgreSQL, AdonisJs
- **Web3:** Ethers.js, Wagmi, Wallet integrations
- **AI:** LLM integrations, agent tooling, Claude and GPT workflows

## Connect

[![Twitter](https://img.shields.io/badge/-@Koinzell27-1DA1F2?style=flat&logo=x&logoColor=white)](https://twitter.com/Koinzell27)
[![LinkedIn](https://img.shields.io/badge/-Carlos%20Quintero-0A66C2?style=flat&logo=linkedin&logoColor=white)](https://linkedin.com/in/carlos-quintero-076a36153)

---
*Building on-chain and with AI*
